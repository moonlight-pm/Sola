...
owner `ws` (`solactl ws …` is first-class — verbs stay in lockstep with the
app), tmux persist on socket `sola-ws`, done-while-unfocused menubar toast.
...
2. Spawn sibling is the only fan-out protocol; a prompt file is the handoff.
   `solactl ws` is the same protocol agents use.
