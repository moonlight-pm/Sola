    ...
    loop {
        ...
        match outcome {
            ...
            Err(error) => {
                ...
                tracing::debug!("ignored request on the redirect listener: {error}");
                continue;
            }
        }
    }
...
fn success_page() -> String {
    page(
        "Signed in to Sola",
        "You're signed in",
        "You can close this tab and go back to Sola.",
        "#1ed760",
    )
}
...
fn failure_page(reason: &str) -> String {
    page(
        ...
        &format!("{reason}. Return to Sola and try again."),
        "#f5717f",
    )
}
...
mod tests {
    ...
    fn stored_token_round_trips_and_tracks_expiry() {
        ...
        let dir = std::env::temp_dir().join(format!("sola-spotify-token-{}", std::process::id()));
