...
fn apply_decl(c: &mut Computed, key: &str, val: &str) {
    match key {
        ...
        "background" | "background-color" => c.bg = parse_color(val),
...
pub fn compute(
    ...
    if el.has_class("label") && c.color.is_none() {
        if parent.is_some_and(|p| p.has_class("is-active")) {
            c.color = Some(Rgba::rgb(0xe9, 0xec, 0xf2));
            c.font_weight = Some(500);
        } else if parent.is_some_and(|p| p.has_class("is-header")) {
            c.color = Some(Rgba::rgb(0x9a, 0xa3, 0xb8));
            c.font_weight = Some(500);
        } else {
            c.color = Some(Rgba::rgb(0xa1, 0xad, 0xc7));
            c.font_weight = Some(400);
        }
        c.font_size = Some(12.0);
        c.font_family = Some("SF Pro Text".into());
    }
    if el.has_class("stage-title") {
        c.color = Some(Rgba::rgb(0xe9, 0xec, 0xf2));
