//! sola-monitor — iced bus inspector. Top toolbar (filter + pause +
//! clear), main row split between a scrollable messages list and a
...
fn main() -> iced::Result {
    ...
    // Kit handles bus connect + subscribe + app-menu publish in one
    // call; the global `sola_kit::bus()` accessor reads from the same
    // slot the kit installs into.
    BusSetup::new(APP_ID)
