    ...
    P::Theme: theme::Base,
{
    ...
    style: theme::Style,
    /// Last Wayland `set_transparent` we sent. `None` until the first sync.
    /// Tiled kit windows paint an opaque fill; the compositor must see a
    /// full `wl_surface.set_opaque_region` or GLES blends ARGB every frame.
    wayland_transparent: Option<bool>,
}
    ...
    P::Theme: theme::Base,
{
    ...
    pub fn new(
        ...
        let mut state = Self {
            ...
            default_theme,
            style,
            wayland_transparent: None,
        };
        state.sync_wayland_opaque_region(window);
        state
    }
    ...
    pub fn synchronize(
        ...
        self.sync_wayland_opaque_region(window);
    ...
    /// Match Wayland opaque-region to the iced window fill.
    ///
    /// `transparent: true` at create time gives an ARGB swapchain (needed
    /// for float CSD). While `background.base` is opaque, mark the surface
    /// fully opaque so the compositor can skip blending / scan out.
    fn sync_wayland_opaque_region(&mut self, window: &Window) {
        let transparent = self.style.background_color.a < 0.999;
        if self.wayland_transparent == Some(transparent) {
            return;
        }
        self.wayland_transparent = Some(transparent);
