            ...
            Event::Keyboard(keyboard::Event::KeyPressed {
                ...
                if let Some(focus) = &mut state.is_focused {
                    ...
                    match key.to_latin(*physical_key) {
                        Some('c')
                            if (state.keyboard_modifiers.command() || state.keyboard_modifiers.logo())
                                && !self.is_secure =>
                        {
                        ...
                        Some('a') if (state.keyboard_modifiers.command() || state.keyboard_modifiers.logo()) => {
                            let cursor_before = state.cursor;
                        ...
                        // ⌘K / Ctrl-K: kill to end — delete from the caret (or a
                        // selection's right edge) to the end of the field. Sola
                        // convention; not in stock iced.
                        Some('k')
                                ...
                                && !self.is_secure =>
                        {
                            ...
                            let from = state.cursor.end(&self.value);
                            let to = self.value.len();
                            if from < to {
                                state.cursor.select_range(from, to);
                                let mut editor =
                                    Editor::new(&mut self.value, &mut state.cursor);
                                editor.delete();
                                let message = (on_input)(editor.contents());
...
// ── Sola kit integration ─────────────────────────────────────────

/// Free constructor mirroring `iced::widget::text_input`, so consumers swap
/// `iced::widget::text_input(..)` for `text_input::text_input(..)` with no
/// other change. Defaults to the kit `Theme`/`Renderer`.
