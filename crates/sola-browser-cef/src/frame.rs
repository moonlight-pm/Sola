...
//! as a fullscreen quad. Shared pipeline lives in `sola_browser_core::shader`.
...
use sola_browser_core::shader::{ImportedTexture, SamplePipeline};
use sola_browser_core::{Cmd, FrameSlot};

use crate::cpu_import::{self, UploadedFrame};
use crate::engine::{CefEngine, CefFrame, InputEvent};
use crate::input;
    ...
    fn update(
        ...
        match event {
            iced::Event::Mouse(m) => {
                ...
                let (x, y) = sola_browser_core::input::project_cursor_i32(
...
impl shader::Pipeline for CefPipeline {
    fn new(device: &wgpu::Device, _queue: &wgpu::Queue, format: wgpu::TextureFormat) -> Self {
        Self {
            sample: SamplePipeline::new(device, format, "cef"),
