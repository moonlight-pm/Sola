...
pub(crate) fn tool_view<'a>(tt: &'a ToolTurn, theme: &Theme) -> Element<'a, Msg> {
    let mut header = row![kit_text::body(format!("⚙ {}", tt.tool))].spacing(SPACE_MD);
    ...
    let mut body = column![header].spacing(SPACE_MD);
    if let Some(summary) = args_summary(&tt.args) {
        body = body.push(kit_text::code(summary).style(kit_text::muted));
    }
    body = body.push(detail);

    sola_kit::components::card::card(body)
        .width(Length::Fill)
        .into()
}
...
fn running<'a>(tt: &'a ToolTurn) -> Element<'a, Msg> {
    column![
        kit_text::caption("running…").style(kit_text::muted),
...
fn diff_view<'a>(path: &str, before: &str, after: &str, theme: &Theme) -> Element<'a, Msg> {
    ...
    let mut lines = column![kit_text::code(path.to_string()).style(kit_text::muted)].spacing(SPACE_XS);
    ...
    container(lines).padding(SPACE_MD).width(Length::Fill).into()
}

fn bash_view<'a>(code: i32, stdout: &str, stderr: &str, theme: &Theme) -> Element<'a, Msg> {
    let p = theme.extended_palette();
    let status_color = if code == 0 {
        p.success.base.color
    } else {
        p.danger.base.color
    };
    let status = text(format!("exit {code}")).size(12).style(move |_t: &Theme| {
        iced::widget::text::Style {
            color: Some(status_color),
        }
    });
    let mut col = column![status].spacing(SPACE_SM);
    if !stdout.is_empty() {
    ...
    container(col).padding(SPACE_MD).width(Length::Fill).into()
}
