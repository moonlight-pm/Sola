...
**As of:** 2026-08-31 (shell Super+H hide + river dead-pid prune on sola-shell; wrapper Edit/links/notify/huddle on master; codecs CEF + autoplay + vault/⌘G; shell Bluetooth + volume; `crates/sola-agent` retired; GPU idle → [`PERFORMANCE.md`](PERFORMANCE.md))

---

## Now

1. **Shell Super+H hide** (this worktree) — composition hide via sticky
...
| Shell | **This worktree.** Super+H hide (no menubar chip; restore Super+Tab / launcher). Dead-pid prune after hard kill. **Installed** `river`+`shell` debug 2026-08-31; menubar remapped. | — |
