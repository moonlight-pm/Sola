...
**As of:** 2026-09-02 (sola-spotify **installed** this worktree: add-to-playlist picker; media keys immediate, page Back/Forward; first pass on master, launcher row live; Super+K live Frame is **card-sized** like menus — software-GL mouse lag; number pad digits **smoked**; screenshot chords → clipboard **installed** `kit`+`shell` release; `cargo make` defaults to release; Arcade watch / singleton / refuse-live-Steam **installed**; Slack huddle camera **smoked**; mail move-rules; browser favicons + popups-as-tabs; Window menu + Super+K; Super+H hide; wrapper Edit/links/notify/huddle; GPU idle → [`PERFORMANCE.md`](PERFORMANCE.md))
