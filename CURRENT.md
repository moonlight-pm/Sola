...
**As of:** 2026-08-31 (screenshot freeze-then-crop + `--app` toplevel capture on sola-shell; Super+H hide + river dead-pid prune; wrapper Edit/links/notify/huddle on master; codecs CEF + autoplay + vault/⌘G; shell Bluetooth + volume; `crates/sola-agent` retired; GPU idle → [`PERFORMANCE.md`](PERFORMANCE.md))

---

## Now

1. **Screenshots (this worktree)** — Super+Shift+4 freeze-then-crop (RGBA still, no dim, GPU-ready before show). `solactl compositor screenshot --app` copies the window scene (`ext-image-copy-capture`); does not raise. **Installed** `river` + `shell` debug 2026-08-31. Desk: freeze picker smoked; `--app sola-arcade` 1434×2132 PNG. Gaps: occluded / Super+H-hidden `--app`; multi-output. Super+H hide remains in this worktree.
...
| Shell | **This worktree.** Window `--app` screenshot (toplevel scene, no raise) **installed** `river` debug 2026-08-31. Super+Shift+4 freeze-then-crop (no dim) **installed** `shell` debug 2026-08-31. Super+H hide. Dead-pid prune; menubar remapped. | — |
