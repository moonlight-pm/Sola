...
**As of:** 2026-08-26 (sola-mail optimistic delete + stable list; wrapper/GPU/arcade as before)
...
5. **sola-mail** — **partial** (worktree `sola-mail`; not merged).
   ...
   **This slice:** delete is optimistic (row leaves immediately; rapid
   `d`/Trash does not wait on IMAP or the next body fetch). A long
   scrolled list keeps its place (keyed rows + silent refresh of the
   loaded window, not a first-page replace).
   **Install:** ask (`install mail`) to dogfood.
6. **Marketing site (sola.computer)** — **teaser live** at
...
7. **sola-workspaces** — **partial** (on master)  
...
9. **Call plane** — on **master**. Host + `solactl compositor` / `session` +
...
12. **Follow-ups (unordered backlog):** create-card; float chrome, D1/D2,
...
| Branch | **master** (workspaces + browser + paint + mail + monitor + terminal + **wrapper**). Worktree `browser-polish` kept open | Feature work in worktrees / Orca workspaces |
...
| Mail | **Worktree `sola-mail`:** same as master, plus optimistic delete and a stable scrolled list (needs `install mail`). Still no HTML engine / attachments | — |
