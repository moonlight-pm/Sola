...
3. **Browser** — codecs CEF (H.264/AAC `canPlayType` `probably`) +
   ...
   **Installed** `browser` (release, 2026-09-04).
   ...
   passkeys stay; no delete) — **installed** `browser` release 2026-09-11, unsmoked.
   **Next:** desk-smoke vault item edit; Google passkey picker; Find + JS dialogs after
   the Continue fix; ⌘⇧R hard reload; vault stay-logged-in; HTTP auth
   persist; group recolor + Steam store trailer.
   Gaps: Google passkey picker unsmoked; DevTools open/close flash; Find unsmoked; confirm-after-fix unsmoked; SVG-only favicons stay globe; vault desk smoke; vault item edit unsmoked; notify no sound / actions / D-Bus.
