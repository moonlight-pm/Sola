...
4. **Shell Bluetooth + volume** (on master) —
   ...
   device pick (`pw-cli ls Node` / `wpctl` /
   ...
   2026-09-10: chip was gone on this desk because `pw-dump` never finished
   (Steam/Factorio `wpctl settings monitor.alsa.enable-external-volume-control`
   stuck ~5h; PipeWire and HDMI/WH-CH520/LifeCam were still up). Graph is
   now `pw-cli ls Node`; chip stays while the PipeWire socket exists.
   **Installed** `shell` release 2026-09-10.
   ...
   volume chip desk-smoke (pw-cli graph; Steam wpctl stall).
...
11. **sola-workspaces** — **partial** (on master)  
   ...
   **Now:** persist + spawn + done toast. Crate/app id `sola-workspaces`.
   ...
   retracts that pane only — tmux `-t` is an exact `=session` so
   `sws-ws-foo` cannot prefix-kill `sws-ws-foo-p`. Installed (self-restart).  
...
| Terminal | **On this branch** (installed 2026-09-02; lib via Workspaces **installed** 2026-09-10): grid selection is neon accent wash (`#3dd6f5` @ 55%). Wash follows scrolled glyphs (`sel_follow`) and TUI CUP-rewrites during drag and page-sized Grok/Codex wheels. Workspaces PTYs share the palette and the follow (**installed** `workspaces` 2026-09-02). First tab / no-source pane starts in `$HOME`. | — |
...
| Calendar | **This worktree.** **Installed** `bus`+`settings`+`calendar` release 2026-09-08 (unsmoked). Settings accounts: Google / iCloud / CalDAV / URL, multiple. | — |
...
| Screens | Five global virtual desktops named **screens**. Default-float; Super+Y dwindle-tiles. Not `sola-workspaces`. |
| Screenshots | Super+Ctrl+3/4/5 → compositor clipboard (`image/png`, Fastest, promised offer). No auto file, no auto Preview. `solactl compositor screenshot` still writes a path. Super+Shift+1…5 sends a window to that screen. |
