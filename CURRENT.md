...
**As of:** 2026-09-23 (NUL in a page title no longer aborts chrome — **installed** `browser` release, unsmoked). 2026-09-23 (workspaces: `workspace.rm` / hover × drop the tab immediately; `tmux kill-session` and `git worktree remove` run off the iced thread so the rail does not freeze — **in this worktree**, not installed). 2026-09-21 (workspaces rail `×N` is the loudest Grok *or* Codex session — Codex counts `type: compacted` in `~/.codex/sessions/**/rollout-*-{id}.jsonl`; Grok still uses session-dir segments/checkpoints. Bots write-fence escape **installed** `bots` release; existing sessions adopt AGENTS.md on the next send). 2026-09-17 (workspaces rail: hover-pencil / double-click is `--title` (label defaults to worktree slug, no `slug ·` prefix); rename keys stay off the PTY; drag-reorder tabs inside a project; drag-reorder project groups; kit `reorder_within_sections` + `Event::Edit` — **installed** `kit`+`workspaces` release, unsmoked). 2026-09-16 (Bots worktree: daemon HTTP + iced + iOS SolaBot). 2026-09-14: OSR HTML5 kanban drag **dogfood good** — unique last_frame + damage continuity + overlay ring; remaining CPU-OSR mitigations parked. 2026-09-12 (vault PUT Bitwarden-Client-Version `2026.8.0` + blob encrypt — **installed** `browser` release. Profile-switch tab leak **installed**. 2026-09-11: workspaces: `pane.send` / `pane.read` / `workspace.exec` use tmux `=session:` pane target — split tabs no longer `send failed`; `solactl browser` **installed** `browser`+`solactl` release; Tertius smoke: background open + `wait --load` + YAML refs + background click-nav to IANA + httpbin fill/radio/checkbox; seat stayed on DHH. vault item **Edit** — **installed** `browser` release, unsmoked; 2026-09-10: workspaces: TUI selection follow during drag + page scroll (Grok/Codex CUP rewrite); Ctrl-D on one split leaf no longer kills the sibling tmux session (exact `=session` target); Codex first-class — `--agent grok|codex`, hooks merged into `~/.codex/hooks.json`, rail rollup; **installed** `workspaces` release; Claude still presence-only; volume device pick: 2s helper timeout so hung pw-dump cannot block set-default — **installed** `shell` release; mail: letter Date + local `HH:MM`; junk is `x` — **installed** `mail` release; number pad digits in the page — OSR `VK_NUMPAD*` not Home/End — **installed** `browser` release, unsmoked; terminal `open` / `xdg-open` on a PDF uses sola-browser, not Chrome — **installed** `browser` release; workspaces persist Grok `session_id` and `grok -r` after reboot, unsmoked; volume chip `pw-dump` JSON strip **installed** `shell` release, unsmoked; sola-calendar **installed** `bus`+`settings`+`calendar` release, unsmoked; 2026-09-04: terminal first tab / no-source pane starts in `$HOME`; kvm Linux listen Super+Tab confirm: virtual-keyboard **key before modifiers**; kernel auto-repeat not injected; sola-spotify: clicks re-activate after Connect inactive; session teardown waits before reconnect; `/me` 429 no longer 2s-polls; artist/album name links; album Save/Follow + artist Follow — **installed** `spotify` release; mail multi-IMAP canonical boxes; send identities **installed** `mail`+`settings` release; browser: vault stay-logged-in + Log out, HTTP auth persist, helper Shutdown-flush, Google passkey document-start intercept, ⌘⇧R hard reload — **installed** `browser` release, desk smoke pending; mail attachments send/receive; kvm Linux listen + CLIP1 text/`image/png`; workspaces `workspace.set --name` moves `.worktrees/<slug>` + `--branch`; self-cleanup: `workspace.rm --worktree` + gone-path reap; grid selection follows scroll; idle omnibox query-strip + Kagi lucky Enter + page ⌘V **smoked**; switcher count marks + grouped notify pile, no cap 20 — **installed** `kit`+`shell` release; shell menubar: pixel-graph stats + 12-band spectrum **raster to a nearest-neighbor image** (not iced canvas 1×1; GLES2 / software GL); notify pile bell + count, Super+Shift+4 freeze keeps open panels, app-menu X from layout — **installed** `shell` release; Super+K live Frame is **card-sized**; DevTools docked panel **smoked**; JS dialogs kit modal; sola-spotify **on master** **installed** `spotify` release; number pad digits **smoked**; screenshot chords → clipboard + promised PNG; `cargo make` defaults to release; Arcade watch / singleton / refuse-live-Steam; Slack huddle camera **smoked**; mail move-rules; GPU idle → [`PERFORMANCE.md`](PERFORMANCE.md))
---
...
0. **Bots** (this worktree) — named informational LLM sessions on this
   ...
   **Installed** `bots`+`solactl`+`browser` (this worktree). HTTP
   ...
   ACP no longer re-sends a turn after disconnect. Chat markdown /
   `verse` fences. **Installed** `bots` release 2026-09-21 (write-fence
   escape in the foundation prompt; existing sessions adopt AGENTS.md
   on the next send). Write fence default is the bot home; Joshua can
   order an escape. Seat steal never lifts. **Next:** session-roll TBD.
...
3. **Browser** — a page title with an interior NUL no longer aborts
   chrome. The Washington Digital Archives download tab
   (`magick-tg-P98v8XUbFBC6uv9Jd58pTwIuN1fnM` plus a trailing NUL) was
   killing `xdg_toplevel.set_title` on launch. Control characters are
   stripped from the window title, CEF title updates, and `session.json`
   restore. **Installed** `browser` release 2026-09-23, unsmoked.
   Codecs CEF (H.264/AAC `canPlayType` `probably`) +
   ...
   **Installed** `browser` (release, 2026-09-04).
   ...
   item edit
   [`docs/specs/2026-09-11-sola-browser-vault-item-edit-design.md`](docs/specs/2026-09-11-sola-browser-vault-item-edit-design.md),
   and tab groups
   ...
   Vault item **Edit** on the record (all shown types; org when writable;
   passkeys stay; no delete) — **installed** `browser` release 2026-09-11;
   PUT uses Bitwarden-Client-Version `2026.8.0` + blob encrypt (**installed**
   2026-09-12). Profile switch does not copy the previous helper’s tabs
   (**installed** 2026-09-12).
   HTML5 OSR drag (Shortcut kanban tickets = pragmatic-dnd Native;
   replica `crates/sola-browser/assets/kanban-dnd.html`) **dogfood
   good** 2026-09-14: unique last_frame, overlay 3-slot ring, skipped-frame
   dirty absorb, GPU uploads overlay dirty. Remaining CPU-OSR cost
   parked
   [`docs/ideas/2026-09-14-osr-cpu-drag-mitigations.md`](docs/ideas/2026-09-14-osr-cpu-drag-mitigations.md)
   (ghost shader quad, overlay restore-ghost, `read_frame` recycle,
   wgpu pad check, credits — not `accelerated_osr`, not a 16 ms timer).
   **Next:** vault item edit Save; Google passkey picker; Find + JS
   dialogs after the Continue fix; ⌘⇧R hard reload; vault stay-logged-in;
   HTTP auth persist; group recolor + Steam store trailer.
   Gaps: Google passkey picker unsmoked; DevTools open/close flash; Find unsmoked; confirm-after-fix unsmoked; SVG-only favicons stay globe; vault desk smoke (Edit Save after client-version fix); notify no sound / actions / D-Bus.
4. **Shell Bluetooth + volume** (on master) —
   ...
   Volume chip no longer hides when `pw-dump` is not JSON (MIDI Through
   unnamed SPA arrays) — **installed** `shell` release 2026-09-08 (unsmoked).
...
7. **sola-terminal** — **partial** (on **master**; `terminal-polish` merged).
   ...
   Window menu **installed** 2026-08-31. First tab / no-source pane
   starts in `$HOME`.
...
9. **sola-mail** — **partial** (on **master**).
   ...
   **Install:** standing OK to `install mail` after each finished round
   (`mail`+`settings` release 2026-09-04; letter time + junk `x` **installed**
   `mail` release 2026-09-10). Self-restarts.
...
11. **sola-workspaces** — **partial** (on master)  
   ...
   **Next:** desk-smoke `solactl workspaces` and grok `-r` after reboot. Polish:
   project recolor. Rail rename / reorder **installed** `kit`+`workspaces`
   release (unsmoked): hover pencil / double-click is `workspace.set --title`
   (rail is that label, or the slug when empty). Drag tabs stay in the
   project; drag group headers; `--before` / `project.reorder`. CLI
   `workspace.set --name` moves `.worktrees/<name>` (id stays);
   `--branch` is `git branch -m`.
   **Installed** `workspaces` release 2026-09-10 (TUI selection follow; split-leaf Ctrl-D; Codex first-class).
   **This slice:** rail `×N` for Codex (rollout `type: compacted`; Grok
   path unchanged) — **installed** `workspaces` release 2026-09-21 (unsmoked). 2026-09-17: rail label is `--title` or the worktree slug; pencil /
   double-click rename; drag-reorder tabs in a project and project groups
   (`workspace.set --before`, `project.reorder`). Rename typing does not
   echo into the PTY. **Installed** `kit`+`workspaces` release 2026-09-17
   (unsmoked). 2026-09-11: pane/window tmux `-t` is `=session:` so split
   tabs `pane.send` / `pane.read` / `workspace.exec` (`pane.read --pane
   ws-sc-17973` after a split).  
   ...
   **Now:** persist + spawn + done toast. Crate/app id `sola-workspaces`.
   ...
   retracts. Quiet `×N` on the workspace row is the loudest Grok or
   Codex session in the tab (Grok: segments / checkpoints; `signals.json`
   can stay 0. Codex: rollout jsonl `type: compacted`). Switching a split attaches every leaf; hover does not spawn. Restart binds tmux by `SOLA_WS_PATH` / cwd
   ...
   `needs attention`.
   Grid selection follows TUI CUP-rewrites (Grok/Codex) while dragging
   and after a page of wheel — remaining lines stay highlighted when
   the top of the range scrolls off. Ctrl-D on a split leaf
   retracts that pane only — tmux session `-t` is exact `=session` so
   `sws-ws-foo` cannot prefix-kill `sws-ws-foo-p`. Pane/window commands
   use `=session:` (installed 2026-09-11).  
...
16. **sola-calendar** — first pass **on master**. Kit month / week / day, local **On This Computer**. Accounts are **Settings → Calendar** only (`Topic::CalendarConfig`): multiple Google, iCloud, CalDAV, and ICS/webcal URLs. Google uses the shipped Desktop OAuth client + secret. Month: date at top-start, full weekday names, events fill the cell (no “+N more” while they fit); sidebar is a kit split (calendars + Today/Tomorrow quickview; no Accounts list); inspector always open on a split (no Close); event editor is a caption-row form (title, when, calendar disc, location, notes); HTML/URL locations are clickable; aliases and colour picker (original name only while editing); hide from sidebar also deactivates (re-show / label / colour in Settings). Freeze [`docs/specs/2026-09-08-sola-calendar-design.md`](docs/specs/2026-09-08-sola-calendar-design.md). **Installed** `bus`+`settings`+`calendar` release 2026-09-08 (unsmoked). Gaps: no invites/alerts; iCloud/CalDAV series edit; URL feeds read-only.
17. **Follow-ups (unordered backlog):** Host `video` extraGroup (uaccess already RW on `/dev/video*`); CEF PipeWire camera needs `rtc_use_pipewire` (build has `use_sysroot=false`). IPC contract handshake
   ([idea](docs/ideas/2026-08-30-ipc-contract-compat.md) — mixed worktree
   installs; not Now); create-card; vault item delete;
   ...
   **desk smoke**; hover × follows the pointer after close; scheme-less
   ...
   longer leave hover-reveal holes; pointer-up outside the page /
   Escape / leave-window cancel — **installed** `browser` release, desk smoke pending.
   Tab-group pockets
...
| Bots | **This worktree.** **Installed** `bots` release 2026-09-21: write-fence escape in the foundation prompt (default home-only; Joshua can order an escape). HTTP SSE `:27419`; iced + iOS clients. Existing sessions adopt AGENTS.md on the next send. ACP dies on `cargo make install bots`. | — |
...
**spotify** after each build, and **browser** after each finished round
on this worktree (OSR drag). User smokes.
