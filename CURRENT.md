...
**As of:** 2026-09-01 (mail move-rules apply on connect + From/To `equals` matches `Name <addr>`; settings rules list+detail + mailbox dest select; Window menu + Super+K overlay **installed** `kit`+`shell`+`paint`+`terminal`; tab-group pocket color on master; screenshot freeze-then-crop + `--app` toplevel capture; Super+H hide + river dead-pid prune; wrapper Edit/links/notify/huddle; codecs CEF + autoplay + vault/⌘G; shell Bluetooth + volume; `crates/sola-agent` retired; GPU idle → [`PERFORMANCE.md`](PERFORMANCE.md))
