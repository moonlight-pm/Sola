...
16. **sola-calendar** — first pass **on this worktree**. Kit month / week / day, local **On This Computer**. Accounts are **Settings → Calendar** (`Topic::CalendarConfig`): multiple Google, iCloud, CalDAV, and ICS/webcal URLs. Freeze [`docs/specs/2026-09-08-sola-calendar-design.md`](docs/specs/2026-09-08-sola-calendar-design.md). **Installed** `bus`+`settings`+`calendar` release 2026-09-08 (unsmoked). **Next:** desk-smoke Settings connect. Gaps: no invites/alerts; Google Desktop OAuth client ID is operator-supplied; iCloud/CalDAV series edit; URL feeds read-only.
