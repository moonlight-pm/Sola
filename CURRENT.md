...
16. **sola-calendar** — first pass **on this worktree**. Kit month / week / day, local **On This Computer**, Google Calendar API (PKCE) + iCloud CalDAV. Freeze [`docs/specs/2026-09-08-sola-calendar-design.md`](docs/specs/2026-09-08-sola-calendar-design.md). Launcher row needs `install shell`. **Not installed.** **Next:** `install calendar` (+ `shell` once) and desk-smoke local create + Google / iCloud. Gaps: no invites/alerts; Google Desktop OAuth client ID is operator-supplied; iCloud series edit; generic CalDAV.
...
| Calendar | **This worktree.** First pass: month/week/day, local store, Google + iCloud connect. **Not installed.** Needs `install calendar` and a one-time `install shell` for the launcher row. | — |
