...
**As of:** 2026-09-01 (Slack huddle camera **smoked**; `XDG_SESSION_TYPE=wayland` from TTY; Chromium `MEDIASTREAM_*` follows `media.json`. Window menu + Super+K overlay **installed** `kit`+`shell`+`paint`+`terminal`; tab-group pocket color on master; screenshot freeze-then-crop + `--app` toplevel capture; mail dest-UID undo + compose table; Super+H hide + river dead-pid prune; wrapper Edit/links/notify/huddle; codecs CEF + autoplay + vault/⌘G; shell Bluetooth + volume; `crates/sola-agent` retired; GPU idle → [`PERFORMANCE.md`](PERFORMANCE.md))
