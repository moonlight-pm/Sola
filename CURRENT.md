...
**As of:** 2026-09-04 (sola-spotify worktree **installed** `spotify` release: clicks re-activate this computer after Connect goes inactive; session teardown waits before reconnect; `/me` 429 no longer 2s-polls. Earlier this worktree: artist/album name links; album Save/Follow + artist Follow. Standing OK to reinstall after each build. **On master** earlier `spotify` release: Back/Forward **persists** 20 back steps; add-to-playlist picker; media keys immediate, page Back/Forward; launcher row live; Super+K live Frame is **card-sized** like menus — software-GL mouse lag; number pad digits **smoked**; screenshot chords → clipboard **installed** `kit`+`shell` release; `cargo make` defaults to release; Arcade watch / singleton / refuse-live-Steam **installed**; Slack huddle camera **smoked**; mail move-rules; browser favicons + popups-as-tabs; Window menu + Super+K; Super+H hide; wrapper Edit/links/notify/huddle; GPU idle → [`PERFORMANCE.md`](PERFORMANCE.md))
