...
16. **sola-calendar** — first pass **on this worktree**. Kit month / week / day, local **On This Computer**. Google / iCloud accounts are **Settings → Calendar** (`Topic::CalendarConfig`). Freeze [`docs/specs/2026-09-08-sola-calendar-design.md`](docs/specs/2026-09-08-sola-calendar-design.md). **Installed** `calendar`+`shell` release 2026-09-08; this slice needs **`install bus settings calendar`**. **Next:** install + desk-smoke Settings connect. Gaps: no invites/alerts; Google Desktop OAuth client ID is operator-supplied; iCloud series edit; generic CalDAV.
...
| Calendar | **This worktree.** App **installed** `calendar`+`shell` 2026-09-08. Accounts moved to **Settings → Calendar** (`Topic::CalendarConfig`) — needs `install bus settings calendar` (unsmoked). | — |
