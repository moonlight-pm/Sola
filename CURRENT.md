...
**As of:** 2026-08-31 (mail dest-UID undo + compose table on master; shell Super+H hide + river dead-pid prune on master; wrapper Edit/links/notify/huddle on master; codecs CEF + autoplay + vault/⌘G; shell Bluetooth + volume; `crates/sola-agent` retired; GPU idle → [`PERFORMANCE.md`](PERFORMANCE.md))

---

## Now

1. **Shell Super+H hide** (on **master**) — composition hide via sticky
   `Topic::AppHidden` (River `hide`, not send-to-back). Restore: Super+Tab
   or launcher (already-running). No menubar chip. SIGKILL during lag
   left zombie sola-shell surfaces (no `closed`); river now prunes
   dead-pid windows. **Installed** `river` + `shell` (debug, 2026-08-31);
   menubar remapped. Gaps: Steam `app_id` variants; no hide animation.
2. **Browser** — codecs CEF (H.264/AAC `canPlayType` `probably`) +
   ...
   **Installed** `browser --release` 2026-08-30. **Next:** Steam store
   ...
   sound / actions / D-Bus.
3. **Shell Bluetooth + volume** (on master) —
   Bluetooth freeze
...
9. **Marketing site (sola.computer)** — **teaser live** at
...
10. **sola-workspaces** — **partial** (on master)  
   ...
   **Now:** persist + spawn + done toast. Crate/app id `sola-workspaces`.
   ...
   (self-restart).  
11. **sola-paint** — default MIME / `solactl open` dest; crop / rotate /
...
12. **Call plane** — on **master**. Host + `solactl compositor` / `session` +
...
13. **sola-arcade / windowed gamescope** — **partial, dogfoodable** (on master)  
...
14. **Distribution follow-through** (when resumed)** — ISO e2e, TZ, tarball.
15. **Follow-ups (unordered backlog):** IPC contract handshake
...
| Shell | **On master.** Super+H hide (no menubar chip; restore Super+Tab / launcher). Dead-pid prune after hard kill. **Installed** `river`+`shell` debug 2026-08-31; menubar remapped. | — |
