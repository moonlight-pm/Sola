...
3. **Browser** — codecs CEF (H.264/AAC `canPlayType` `probably`) +
   ...
   **Installed** `browser` (release, 2026-09-04).
   Number pad digits in the page were Home/End (iced unmodified `key`);
   OSR now sends `VK_NUMPAD*` + CHAR. Omnibox arrows use the produced
   key so KP_2 does not steal history-tray ↓. **Installed** `browser`
   release 2026-09-05.
   Freeze
   ...
   **Next:** desk-smoke numpad digits in a page field + omnibox; Google
   passkey picker; Find + JS dialogs after the Continue fix; ⌘⇧R hard
   reload; vault stay-logged-in; HTTP auth persist; group recolor +
   Steam store trailer.
   Gaps: numpad in page unsmoked; Google passkey picker unsmoked; DevTools open/close flash; Find unsmoked; confirm-after-fix unsmoked; SVG-only favicons stay globe; vault desk smoke; no item edit; notify no sound / actions / D-Bus.
...
16. **Follow-ups (unordered backlog):** Host `video` extraGroup (uaccess already RW on `/dev/video*`); CEF PipeWire camera needs `rtc_use_pipewire` (build has `use_sysroot=false`). IPC contract handshake
   ...
   **desk smoke**; hover × follows the pointer after close; scheme-less
   ...
   (`solactl browser`, same strip, isolation is a group + skill — locked,
   not built). Passkey **create** smoked. Page
   ...
   **installed**: flush members, hairline rim, header drag moves the
   ...
   ghost. **⌘⌥G** new group **installed** 2026-08-29 (`⌘G` is Find Next). Pocket color (edit-mode swatch) **installed** `kit`+`browser --release` 2026-08-31.
...
| Browser agent control | Same strip as the human (all tabs). Isolation is an ordinary tab group the agent may create. No agent-only tab type, hidden session, or Ask-AI omnibox. Workflow (pocket name, do not steal focus) is a **skill**, not chrome. Face when built: `solactl browser` (owner `browser`), first-class like Workspaces. Confirm remains **D3**. |
