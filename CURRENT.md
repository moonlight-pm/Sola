...
11. **sola-workspaces** — **partial** (on master)  
   ...
   **This slice:** pane/window tmux `-t` is `=session:` so `pane.send` /
   `pane.read` / `workspace.exec` work on a split tab (bare `=session`
   was `can't find pane` / `send failed`). **Installed** `workspaces`
   release 2026-09-11 (`pane.read --pane ws-sc-17973` after a split).  
