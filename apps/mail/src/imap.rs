...
use crate::rules::{Folder, MailRule, MessageBody, MessageSummary};
...
impl ImapClient {
    ...
    pub fn connect(config: &MailConfig) -> anyhow::Result<Self> {
...
fn new_session(config: &MailConfig) -> anyhow::Result<ImapSession> {
...
fn build_imap_search(conditions: &[crate::rules::MailRuleCondition]) -> String {
    conditions
        .iter()
        .filter_map(|c| {
