...
impl MailConfig {
    ...
    /// Serialize the full config to a TOML string.
    pub fn serialize(&self) -> String {
        ...
        output.push_str(&format!(
            "email = {}\n",
            toml::Value::String(self.email.clone())
        ));
        output.push_str(&format!(
            "imap_host = {}\n",
            toml::Value::String(self.imap_host.clone())
        ));
        output.push_str(&format!("imap_port = {}\n", self.imap_port));
        output.push_str(&format!(
            "smtp_host = {}\n",
            toml::Value::String(self.smtp_host.clone())
        ));
        output.push_str(&format!("smtp_port = {}\n", self.smtp_port));
        output.push_str(&format!(
            "username = {}\n",
        ...
        for rule in &self.rules {
            ...
            output.push_str(&format!(
                "name = {}\n",
                toml::Value::String(rule.name.clone())
            ));
            output.push_str(&format!(
                "action = {}\n",
            ...
            if !rule.conditions.is_empty() {
                ...
                for cond in &rule.conditions {
                    output.push_str(&format!(
                        "  {{ field = {}, match = {}, value = {} }},\n",
                        toml::Value::String(cond.field.clone()),
                        toml::Value::String(cond.match_type.clone()),
                        toml::Value::String(cond.value.clone()),
                    ));
                }
                output.push_str("]\n");
            }
...
"#;
    ...
    fn serialize_full_config() {
        let config = MailConfig {
            ...
            rules: vec![
                MailRule {
                    ...
                    action: "smart_mailbox".to_string(),
                    dest: None,
                ...
                MailRule {
                    ...
                    dest: Some("Newsletters".to_string()),
                    conditions: vec![MailRuleCondition {
                        field: "from".to_string(),
        ...
        assert!(output.contains("imap_host = \"mail.example.com\""));
        assert!(output.contains("imap_port = 993"));
        assert!(output.contains("smtp_host = \"smtp.example.com\""));
        assert!(output.contains("smtp_port = 587"));
        assert!(output.contains("[[rule]]"));
        assert!(output.contains("name = \"GitHub\""));
        assert!(output.contains("action = \"smart_mailbox\""));
        assert!(output.contains("name = \"Move newsletters\""));
        assert!(output.contains("dest = \"Newsletters\""));
        // Does NOT contain [wicket] section
        assert!(!output.contains("[wicket]"));

        // Round-trip: parse the output back
        let parsed = MailConfig::parse(&output).unwrap();
        assert_eq!(parsed.email, "user@example.com");
        assert_eq!(parsed.imap_host, "mail.example.com");
        ...
        assert_eq!(parsed.rules[0].name, "GitHub");
        assert_eq!(parsed.rules[1].dest.as_deref(), Some("Newsletters"));
    }
}
