...
| browser | sola-browser chrome + CEF (single crate) | partial | [cef port](specs/2026-05-04-cef-port-design.md); [profiles D8](specs/2026-08-10-sola-browser-profiles-design.md); [manual](manual/sola-browser.md) | local: chrome + tab switch solid; Profiles menubar; vault fill list taller; WebAuthn passkey get() intercept (Bitwarden FIDO2) | **Gaps:** passkey registration + multi-cred picker not shipped; passkey needs desk dogfood; loading/history flags not from CEF yet; engine OSR polish; manual still limited | partial |
