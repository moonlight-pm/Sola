...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening](plans/2026-08-09-sola-browser-hardening.md); [Bitwarden D7](specs/2026-08-10-sola-browser-bitwarden-design.md); [profiles D8](specs/2026-08-10-sola-browser-profiles-design.md) | local; re-dogfood YouTube login after D8 wipe | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus, float CSD, dma-buf paint (multi-plane RGB), NewFrame coalesce, CSS+DPR, buffer epoch, stop, history nav, copy/paste, Bitwarden unlock+match+fill. **D8 profiles:** UUID **Primary**, `profiles/<id>/` data+cache, `profiles.json`, per-profile `session.json`, vault at `config/sola/browser/vault.json`, first-run wipe of flat legacy paths. **Gaps:** no profile switcher; cookie stickiness re-dogfood after wipe; vault session not persisted; no downloads/visit-history UI; NV12 not GPU-imported; B1/B4/B5; OpenUrl→Helium (D3); no operator manual. CEF gone | no |
