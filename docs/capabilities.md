...
| kit | sola-kit components + storybook | partial | [kit](specs/2026-04-30-sola-kit-design.md), graphite DS, [unified sidebar](specs/2026-08-13-unified-sidebar-design.md) | storybook + apps (sidebar etch on master; hover × opaque chip + no phantom `↓ N` on auto-filled tab strips on `naturalethic/browser-polish`) | **Select** hangs as a darker inset card (tabs peek around it). Default list chrome is etch (muted idle, lip + inset active, hover-only opaque × chip, `SidebarDensity`). Overflow chips only when `section_scroll` is wired and the viewport is measured. Card chrome unchanged. **vertical_tabs** / `TabDescriptor` removed. Storybook Select page; other pages still lag Overview composition | no |
...
| Area | Notes |
|------|--------|
| **browser polish** | 2026-08-15: tab strip no longer shows a false `↓ N` overflow chip (auto-fill used a 480px viewport); chrome singleton (`chrome.sock`) + orphan-only helper reap + helper respawn; passkey **create**; Gemini Exchange passkey **get**; downloads flatten. 2026-08-14: downloads auto-save + toolbar list. Earlier: tab reorder + width-aware titles; Bitwarden **cards** |
