...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening](plans/2026-08-09-sola-browser-hardening.md); [Bitwarden D7](specs/2026-08-10-sola-browser-bitwarden-design.md); [profiles D8](specs/2026-08-10-sola-browser-profiles-design.md); [paint investigation](plans/2026-08-10-browser-paint-investigation.md) | local; YouTube homepage scroll | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus, float CSD, dma-buf paint (multi-plane RGB), NewFrame coalesce, CSS+DPR, buffer epoch, stop, history nav, copy/paste, Bitwarden unlock+match+fill. **D8 profiles:** UUID **Primary**, `profiles/<id>/` data+cache. **Paint:** SEGV+EMFILE fixed; **telem**; latest-wins mailbox; hide inactive views; owned blit+Wait; **headless render_buffer hijack** (no auto-release race). **Gaps:** user re-dogfood scroll feel / residual blur; no profile switcher; cookie stickiness; vault session; downloads/history UI; NV12; B1/B4/B5; OpenUrl→Helium (D3); no operator manual. CEF gone | no |
