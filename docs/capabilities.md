...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening plan](plans/2026-08-09-sola-browser-hardening.md); freezes historical | local install 2026-08-09 (`/opt/sola/bin/sola-browser`); unit tests chrome util/integration | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus (⌘T/W/L/R/←/→/Q + Edit), float CSD, dma-buf paint, copy/paste bridges, last-tab→blank, cmd-click→bg tab, theme. **Gaps (see plan):** bg-tab frame thrash (B1); GPU release order (B2); multi-plane drop (B3); IME/keycode (B4); middle-click dead (B5); no find/zoom/bookmarks/history/downloads/profiles/devtools/stop UI; OpenUrl→Helium not sola-browser; title/url tick 250ms; no operator manual. CEF gone (`pre-cef-removal`) | no |
