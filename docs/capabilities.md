...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening plan](plans/2026-08-09-sola-browser-hardening.md); freezes historical | local install 2026-08-09 (`/opt/sola/bin/sola-browser`); unit tests chrome util/integration | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus (⌘T/W/L/R/←/→/Q + Edit), float CSD, dma-buf paint, copy/paste bridges, last-tab→blank, cmd-click→bg tab, theme. **Product bar (D4):** stop, downloads, history+restore, Bitwarden, high polish. **Session tabs restored** (`~/.config/sola/browser-session.json`). **D7:** first-party Bitwarden UX. **Gaps:** engine B1–B5; tab-switch flicker residual; no stop/downloads/visit-history UI/Bitwarden; no find/zoom/bookmarks/devtools; OpenUrl→Helium (D3); no operator manual. CEF gone (`pre-cef-removal`) | no |
