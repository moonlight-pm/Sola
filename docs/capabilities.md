...
| browser | sola-browser chrome + CEF (single crate) | partial | [cef port](specs/2026-05-04-cef-port-design.md); [profiles D8](specs/2026-08-10-sola-browser-profiles-design.md); [manual](manual/sola-browser.md) | local: chrome + tab switch + Profiles; Bitwarden unlock (release install); fill panel after unlock; WebAuthn **get** passkey picker dogfooded on Google | **Gaps:** passkey registration not shipped; loading/history flags not from CEF; engine OSR polish (input/scroll/quirks); manual still limited | partial |
