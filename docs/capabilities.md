...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening plan](plans/2026-08-09-sola-browser-hardening.md); [Bitwarden D7](specs/2026-08-10-sola-browser-bitwarden-design.md) | local install 2026-08-10; Google/YouTube sign-in dogfood (typing/caret OK); Bitwarden unlock + match/fill dogfood | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus, float CSD, dma-buf paint (opaque + dark fallback), **one live hold** (no park/retire multi-hold), NewFrame coalesce, CSS+DPR HiDPI (compositor scale; 2560 edge cap), buffer **epoch** on load-started (safe release after navigate), stop (fixed-width ↻/× + Escape), back/forward only when history allows, multi-plane **released** (not imported), copy/paste, last-tab→blank, cmd-click bg tab, theme, session tabs, modified_key typing (`@` etc.). **D7 Bitwarden (in-browser):** official cloud login (incl. `newDeviceOtp`), sync, unlocked panel **match picker** for active tab URL, **page fill** via WPE `evaluate_javascript` (`Cmd::EvaluateJs`). **Product bar (D4 remaining):** downloads, visit history UI, Bitwarden polish (persist session, idle lock, auto-offer), browser polish. **Gaps:** vault session **not** persisted across browser restart; no idle lock / lock cmd; no single-match auto-offer; fill is best-effort field heuristics (not extension-class on every site); no multi-plane import (video); no downloads/visit-history UI; B1/B4/B5; tab-switch may flash (no park); residual font vs Chromium (P1.6 partial); OpenUrl→Helium (D3); no operator manual. CEF gone (`pre-cef-removal`) | no |
...
| Area | Notes |
|------|--------|
| **sola-browser** | 2026-08-10: paint/input pipeline dogfood (sign-in OK); one-hold dma-buf; NewFrame coalesce; CSS+DPR; buffer epoch; stop; history nav; multi-plane release; session restore; CEF removed; Bitwarden unlock + match picker + page fill (no session persist) |
