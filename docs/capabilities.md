...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening plan](plans/2026-08-09-sola-browser-hardening.md); freezes historical | local install 2026-08-10 (`/opt/sola/bin/sola-browser`); unit tests chrome util/integration | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus (⌘T/W/L/R/←/→/Q + Edit), float CSD, dma-buf paint (opaque content + dark fallback — no desktop hole), park-on-switch + restore on close-active, optimistic omnibox URL on tab switch, copy/paste bridges, last-tab→blank, cmd-click→bg tab, theme, session tabs (`browser-session.json`). **Product bar (D4):** stop, downloads, history+restore, Bitwarden, high polish. **D7:** first-party Bitwarden UX. **Gaps:** no stop control yet; no downloads/visit-history UI/Bitwarden; engine B1–B5; residual tab-switch flicker; no find/zoom/bookmarks/devtools; OpenUrl→Helium (D3); no operator manual. CEF gone (`pre-cef-removal`) | no |
...
| Area | Notes |
|------|--------|
| **sola-browser** | 2026-08-10: opaque shader α=1 + dark fallback; close-tab park restore when `active` cleared; omnibox URL from cache on switch (no 250 ms lag); session restore; multi-tab park; self-watch; CEF removed |
