...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening plan](plans/2026-08-09-sola-browser-hardening.md); freezes historical | local install 2026-08-09 (`/opt/sola/bin/sola-browser`); unit tests chrome util/integration | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus (⌘T/W/L/R/←/→/Q + Edit), float CSD, dma-buf paint, copy/paste bridges, last-tab→blank, cmd-click→bg tab, theme. **Product bar (D4):** stop, downloads, history+restore, Bitwarden, high polish — **none shipped yet**. **D7:** first-party Bitwarden UX (SDK + inject), not Chrome extension host. **Gaps:** engine B1–B5; no stop/downloads/history/Bitwarden; no find/zoom/bookmarks UI/devtools; OpenUrl→Helium (D3); no operator manual. CEF gone (`pre-cef-removal`) | no |
