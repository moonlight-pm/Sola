...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening plan](plans/2026-08-09-sola-browser-hardening.md); freezes historical | local install 2026-08-10 (`/opt/sola/bin/sola-browser`); unit tests chrome util/integration | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus, float CSD, dma-buf paint (opaque + dark fallback), **retire-ring buffer holds**, park-on-switch + close restore, activate 1px repaint nudge, optimistic omnibox, stop (↻/× + Escape), copy/paste, last-tab→blank, cmd-click bg tab, theme, session tabs. **Product bar (D4 remaining):** downloads, visit history UI, Bitwarden, high polish. **D7:** first-party Bitwarden UX. **Gaps:** no downloads/visit-history UI/Bitwarden; engine B1/B3–B5 (B2 mitigated); residual switch flash possible; no find/zoom/bookmarks/devtools; OpenUrl→Helium (D3); no operator manual. CEF gone (`pre-cef-removal`) | no |
