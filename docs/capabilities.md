...
| workspaces | sola-workspaces: project groups + workspaces + agent-aware PTYs | partial | [freeze](specs/2026-08-13-sola-agent-terminal-design.md), [idea](ideas/2026-08-12-sola-agent-terminal.md), [call plane](specs/2026-08-13-sola-call-plane-design.md) | on master + `workspaces-polish` install; rail, splits, drop-project, dead-pane, `×N` smoked. `solactl ws` unsmoked. Add project expands `~`; groups stack at the top; no agent label on the workspace row. Sibling hover close is kit `on_close` (×). **Drop Project** menu-only (`project.rm`); worktrees stay. ⌘W closes the focused pane. Kit splits (⌘⇧↓ / ⌘⇧→); leaf rows follow presence (`grok`/`shell`) only after a split. Dead last pane: **Start new shell**; split leaf exit retracts. Switching a split attaches every leaf; hover does not spawn. Quiet `×N` only on a Grok leaf (session dir segments/checkpoints, then `signals.json` — that field can stay 0). Restart attaches only when `SOLA_WS_PATH` / cwd matches; leftover sessions quarantined. Shell builtin **Workspaces** (`lucide/folders`). Working ring spins (kit ms phase) | **Gaps:** no rename/recolor/reorder; drop does not `git worktree remove`; Claude presence-only. Distinct from `sola-agent`. | no |
...
| Area | Notes |
|------|--------|
| **workspaces** | 2026-08-18: `×N` only on a Grok leaf (segments/checkpoints; `signals.json` can stay 0); leaf labels follow presence; switch attaches every split leaf (hover does not spawn). Ctrl-D on a split leaf retracts (Start new shell only on the last pane). Kit pane splits (⌘⇧↓ / ⌘⇧→); ⌘W close pane; Drop Project menu-only; leaf rows after split. Restart binds tmux by path (`SOLA_WS_PATH`); leftover / externally-deleted sessions quarantined. Earlier today: ⌘T spawn / ⌘N new project; working ring spins (kit ms phase). Launcher builtin; kit hover × on siblings (not root); Add project expands `~`; groups stack at the top; agent name off the workspace row. 2026-08-15: landed on master. 2026-08-14: renamed from sola-agent-terminal; owner `ws` (`solactl ws`); config `~/.config/sola/workspaces/` |
