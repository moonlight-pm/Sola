...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening](plans/2026-08-09-sola-browser-hardening.md); [Bitwarden D7](specs/2026-08-10-sola-browser-bitwarden-design.md); [profiles D8](specs/2026-08-10-sola-browser-profiles-design.md); [content plane](specs/2026-08-11-sola-browser-content-plane-design.md); [paint investigation](plans/2026-08-10-browser-paint-investigation.md) | local; YouTube homepage + scroll | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus, float CSD, **content-plane paint** (Wayland subsurface + dma-buf; default `plane`), D8 profiles, Bitwarden unlock+match+fill, stop/history nav, copy/paste. **Paint:** SEGV/EMFILE fixed; plane G1–G3 dogfood (homepage grid + scroll delta); empty input region → iced/WPE input; fallback `SOLA_BROWSER_CONTENT=import`. **Gaps:** user residual blur/feel; no profile switcher; cookie stickiness; vault session; downloads/history UI; NV12; B1/B4/B5; system http→Helium (D3); no operator manual. CEF gone | no |
