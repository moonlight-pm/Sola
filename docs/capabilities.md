...
| browser | sola-browser chrome + CEF (single crate) | partial | [cef port](specs/2026-05-04-cef-port-design.md); chrome from naturalethic/browser | local dogfood: chrome + tab switch solid | **Gaps:** loading/history flags not from CEF yet; engine OSR polish (input/scroll/quirks) still open; no operator manual; WPE multi-crate path removed after failed dogfood | no |
