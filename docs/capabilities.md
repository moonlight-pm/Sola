...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening](plans/2026-08-09-sola-browser-hardening.md); [Bitwarden D7](specs/2026-08-10-sola-browser-bitwarden-design.md); [profiles D8](specs/2026-08-10-sola-browser-profiles-design.md); [content plane](specs/2026-08-11-sola-browser-content-plane-design.md); [paint investigation](plans/2026-08-10-browser-paint-investigation.md) | local; YT homepage paints + scrolls | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus, float CSD, **content-plane paint** (default `plane`: subsurface + dma-buf; hold to compositor Release; frame-callback paced attach; **FrameDone after Wayland frame cb**; never force-release on-screen buffers; 2× content scale; empty input region), D8 profiles, Bitwarden unlock+match+fill, stop/history nav, copy/paste. **Paint:** SEGV/EMFILE fixed; plane G1–G4; force-release removed (blur better); FrameDone timing aligned with present. **Gaps:** **daily-driver YT hard-scroll** (black/flicker re-dogfood after FrameDone fix); residual soft text; no profile switcher; cookie stickiness; vault session; downloads/history UI; NV12; B1/B4/B5; system http→Helium (D3); no operator manual. CEF gone | no |
...
| Area | Notes |
|------|--------|
| **sola-browser** | 2026-08-11: content plane; FrameDone after present (not 60 Hz); frame-paced attach; blur better; re-dogfood black/flicker; D8; Bitwarden |
