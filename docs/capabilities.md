...
| kit | sola-kit components + storybook | partial | [kit](specs/2026-04-30-sola-kit-design.md), graphite DS, [unified sidebar](specs/2026-08-13-unified-sidebar-design.md) | storybook + apps (sidebar etch **code-complete**). Overview/Button/Field/Form/Select desks + Select chrome fix on `naturalethic/kit-design` (**needs install**) | Select menu is a raised popover matching the trigger. Selection seed is graphite `#2c333e` (neon stays full-chroma; no darkened-cyan mixes). Panel fills are card-whisper only. Default list chrome is etch. Storybook header uses kit select on `CHROME_SURFACE`. Card / Theme / remaining lab pages still lag Overview composition | no |
