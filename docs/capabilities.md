...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening plan](plans/2026-08-09-sola-browser-hardening.md); freezes historical | local install 2026-08-10 (`/opt/sola/bin/sola-browser`); unit tests chrome util/integration | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus, float CSD, dma-buf paint (opaque + dark fallback + size heal), retire-ring holds, park restore, activate repaint, optimistic omnibox, stop (fixed-width ↻/× + Escape), **back/forward disabled without history**, multi-plane frames released (no pool leak), copy/paste, last-tab→blank, cmd-click bg tab, theme, session tabs. **Product bar (D4 remaining):** downloads, visit history UI, Bitwarden, high polish. **D7:** first-party Bitwarden UX. **Gaps:** no multi-plane import (video may blank/stutter); no downloads/visit-history UI/Bitwarden; B1/B4/B5; residual switch flash possible; no find/zoom/bookmarks/devtools; OpenUrl→Helium (D3); no operator manual. CEF gone (`pre-cef-removal`) | no |
...
| Area | Notes |
|------|--------|
| **sola-browser** | 2026-08-10: opaque paint; retire-ring; size heal; stop; history-aware back/forward; fixed reload width; multi-plane release (YouTube pool crash); session restore; CEF removed |
