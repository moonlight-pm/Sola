...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening](plans/2026-08-09-sola-browser-hardening.md); [Bitwarden D7](specs/2026-08-10-sola-browser-bitwarden-design.md); [profiles D8](specs/2026-08-10-sola-browser-profiles-design.md) | local; YouTube/Google paint+sign-in dogfood; Bitwarden match/fill | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus, float CSD, dma-buf paint (opaque + multi-plane **RGB** import), NewFrame coalesce, CSS+DPR, buffer epoch, stop, history nav chrome, copy/paste, session tabs (flat config today), Bitwarden unlock+match+fill. **D8 profiles:** freeze only — UUID `Primary`, paths under `profiles/<id>/`, config `~/.config/sola/browser/`, no migration. **Gaps:** D8 not implemented (flat WebKit dir + root `browser-*.json`); cookie stickiness flaky until D8; vault session not persisted; no downloads/visit-history UI; NV12 multi-plane not GPU-imported; B1/B4/B5; OpenUrl→Helium (D3); no operator manual. CEF gone | no |
...
| Area | Notes |
|------|--------|
| **sola-browser** | 2026-08-10: paint/input + multi-plane RGB; Bitwarden match/fill; **D8 profiles freeze** (implement next); cookie/path cleanup pending |
