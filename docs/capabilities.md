...
| browser | Browser chrome + WPE engine (single crate) | partial | [hardening plan](plans/2026-08-09-sola-browser-hardening.md); freezes historical | local install 2026-08-09 (`/opt/sola/bin/sola-browser`); unit tests chrome util/integration | **Shipped subset:** multi-tab chrome, omnibox+Kagi, bus menus (⌘T/W/L/R/←/→/Q + Edit), float CSD, dma-buf paint, copy/paste bridges, last-tab→blank, cmd-click→bg tab, theme. **Product bar (D4):** stop loading, downloads, history+session restore, Bitwarden (extension-class), high polish — **none shipped yet**. **Gaps:** engine B1–B5; no stop/downloads/history/Bitwarden; no find/zoom/bookmarks UI/devtools; OpenUrl→Helium (D3); Bitwarden approach open (**D7**); no operator manual. CEF gone (`pre-cef-removal`) | no |
