...
Kernel auto-repeat (`EV_KEY` value 2) is **not** injected — River would
stack press counts and leave Super/Alt stuck (terminal drops Super+letter
or prints ESC+char). The compositor generates its own repeat.
