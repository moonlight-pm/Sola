...
shader/iced integration honest. Both crates sit outside the cargo
workspace at `crates/sola-browser-wpe/` and `crates/sola-browser-cef/`
because their `iced` dependency drags in `smithay-clipboard` →
`wayland-sys` with the `dlopen` feature, which would unify
across the workspace and break [[sola-river]]'s direct-link
wayland.

**Status (2026-05-22): pause point.** WPE is primary, CEF is at
feature parity. Neither is yet a production-grade browser — the
chrome is minimal, there's no profile/cookie persistence, no
keyboard shortcuts. The architecture is proven; what remains is
polish + the deeper browser feature set (bookmarks, downloads,
history, devtools UI, etc.). We are pausing browser work to
focus on other parts of Sola.
...
- **Keyboard shortcuts (Ctrl-T new tab, Ctrl-W close, Ctrl-Tab
  switch, Ctrl-L focus URL, etc.).** Deferred deliberately — sola
  will customise all keybinds at a higher level, so we don't want
  them hardcoded per-app.
...
6. **Keyboard shortcuts** — once Sola's app-level shortcut
   routing is decided (see [[Topics]] / general sola design).
