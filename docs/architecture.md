...
| `crates/sola-calendar` | Kit-native calendar. Local **On This Computer** plus Google Calendar API v3 and iCloud CalDAV. Accounts from persistent `Topic::CalendarConfig` (Settings → Calendar). Events `~/.local/state/sola/calendar/store.json`; view prefs `~/.config/sola/calendar/settings.json`. Launcher **Calendar** (`lucide/calendar`). |
