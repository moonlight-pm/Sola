...
| Profiles (D8) | Registry `~/.local/share/sola/browser/profiles.json`; per-id data/cache under `profiles/<uuid>/`; session `session.json`; CEF user data `profiles/<uuid>/cef/`; menubar Profiles switch re-execs |
