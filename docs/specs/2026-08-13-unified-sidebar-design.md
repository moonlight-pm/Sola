...
| | |
|--|--|
| **Implementation** | Kit `SidebarDensity` + etch list chrome; **kit-owned gesture** (`sidebar::State` + `Event::{Activate,ToggleSection,Edit,Drop,Resize}`); browser/terminal/workspaces on `SidebarPanel`; labeled headers participate in Morph2 reorder; `.reorder_within_sections()` keeps items in group; `SidebarItem::leading` 16px slot (browser favicons); `vertical_tabs*` deleted |
