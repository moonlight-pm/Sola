...
**Dogfood:** `shell` installed debug 2026-08-29; 12-band spectrum **installed** `river`+`shell` release 2026-09-02 and desk-smoked on Spotify (presence-band, punch gate, phrase gaps). 2026-09-10: device pick was stuck behind a hung `pw-dump` (PipeWire control stall; Factorio `wpctl settings` sibling). Helpers now time out at 2s; set-default no longer waits on the dump — **installed** `shell` release. Desk-smoke of sink/source switch pending (`wpctl` still does not answer on this desk). Still pending: volume keys vs chip.  
**Gaps:** no per-app streams; no Bluetooth profile/codec; host PipeWire + WirePlumber (`pw-dump`, `wpctl`, `pw-cat`); existing streams may stay on the old sink until they reconnect
...
| Rule | Choice |
|------|--------|
| Icon | Right cluster, **right of Bluetooth** / immediately **left of the stats** (system-control, not a metric). Full-height `bar_button`. **No speaker glyph** — the **12-band LED spectrum** is the chip (same 2×2 dither as CPU/MEM; each band is 3 LED columns; ~3× the stats-graph width). Phosphor stack is green → teal → azure → violet → amber by height. Mute dims the bars. |
...
```
[mail?] [notify?] [bluetooth?]  [████████████]  CPU …  clock
```
