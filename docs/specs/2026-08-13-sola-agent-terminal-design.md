...
**Dogfood:** app installed; rail, splits, drop-project, dead-pane, and `×N` smoked on `workspaces-polish`. Session-reclaim fix installed. Super-chord no longer latches LOGO (⌘T/⌘V used to kill typing until quit). Exiting Grok back to the shell idles the mark (grey disc; was stuck on done). Grid selection follow (`sel_follow`) installed `workspaces`+`terminal` 2026-09-02 (desk smoke). `solactl workspaces` still needs a desk smoke. Grok `-r` after reboot is in code, not smoked.  
**Gaps:** UI rename modal / recolor / reorder; Claude presence-only (D4). CLI `workspace.set --name` moves the worktree; `--branch` renames HEAD. Grok `-r` after reboot unsmoked.
