...
**Gaps:** UI rename modal / recolor / reorder; Claude presence-only (D4). Codex first-class (spawn/exec/hooks) as of 2026-09-09; first Codex session may need `/hooks` trust. CLI `workspace.set --name` moves the worktree; `--branch` renames HEAD. Grok `-r` after reboot unsmoked.
